...
- **Confusing intensities with probabilities.** `q_rs` is a rate; don't drop it into a discrete-time transition matrix as if it were `p_rs`. Convert via the matrix exponential (`pmatrix.msm()` does this for msm fits) over a defined interval. `Exp(uQ)` is exact only when `Q` is constant over the interval — for one state's competing exits it reduces to `p_k = (ΔH_k/ΔH_tot)·(1 − exp(−ΔH_tot))`, which stays exact whenever the cause-specific hazards keep a fixed ratio across the interval (the receiving end of `survival-analysis-hta`'s conversion guidance). With genuinely time-varying intensities, use piecewise-constant `Q` over sub-intervals, the Aalen-Johansen/product-integral route, or individual simulation (hesim), rather than one long-interval matrix exponential.
