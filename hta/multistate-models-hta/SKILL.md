...
- **Confusing intensities with probabilities.** `q_rs` is a rate; don't drop it into a discrete-time transition matrix as if it were `p_rs`. Convert via the matrix exponential (`pmatrix.msm()` does this for msm fits) over a defined interval. `Exp(uQ)` is exact when `Q` is constant over the interval (more generally, when all intensities scale by a common time function — then with the *integrated* intensity matrix in place of `uQ`). Mind what each quantity means: `Exp(uQ)`'s off-diagonals are *occupancy* probabilities (the destination can itself be left again within the interval), whereas the probability that the **first exit within the interval** from a state is via cause k is `p_k = (ΔH_k/ΔH_tot)·(1 − exp(−ΔH_tot))` — exact whenever the cause-specific hazards keep a fixed ratio across the interval (the receiving end of `survival-analysis-hta`'s conversion guidance); the two coincide only when the destinations are not themselves left within the interval. With genuinely time-varying intensities, use piecewise-constant `Q` over sub-intervals, the Aalen-Johansen/product-integral route, or individual simulation (hesim), rather than one long-interval matrix exponential.
