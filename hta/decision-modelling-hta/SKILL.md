...
PSA in heemod means: re-specify the relevant parameters via `define_psa()` with a resampling distribution attached to each as a **formula** (`param ~ distribution(...)`), then call `run_psa(model, psa, N)`. Use heemod's built-in density functions — but **watch the parameterisations, which differ by distribution**: `gamma(mean, sd)` and `normal(mean, sd)` take mean/sd directly, `lognormal()` accepts natural- or log-scale, `binomial(prob, size)` takes a point probability plus effective sample size, and `beta(shape1, shape2)` takes **shape parameters, not mean/sd** (a common error). For a probability where you only have a mean and sd, `binomial(prob, size)` is the easier idiomatic choice. `define_distribution()` takes a user-supplied *quantile* function, not a density; a vector of MCMC draws goes to `use_distribution()`, which wraps it. Neither is the normal path — see `references/bayesian-transition-parameters.md` for which of the two a posterior actually needs, and for what heemod does and does not preserve of a joint posterior. The reference files give the full table and worked calls.

For distribution *choice* (which family for which parameter type) — **defer to the `nice-economic-evaluation` skill's conventions** if there's any conflict, rather than re-deriving them here. The one structural rule worth stating up front: the several outgoing probabilities from a single state share a simplex constraint (must sum to ≤1), so model them jointly with `multinomial(...)` rather than as independent draws. Where those probabilities were *estimated from transition counts*, the Multinomial-Dirichlet posterior is the distribution to resample from and is available in closed form — see `references/bayesian-transition-parameters.md`, which also covers applying a literature relative risk to a transition matrix without producing impossible probabilities, and feeding an evidence-synthesis or MCMC posterior through the model instead of refitting a parametric distribution to it. Correlated parameters from the same regression (e.g. a survival model's joint parameter uncertainty, or cause-specific log-rates) need a correlation structure passed as the `correlation =` argument of `define_psa()`. That argument accepts **either** the output of `define_correlation(par1, par2, rho, ...)` **or** a plain correlation matrix directly (`correlation = matrix(...)`) — both are supported (heemod CRAN docs; Antoine Filipović-Pierucci et al., *heemod* JSS/arXiv §4.1, verified 2026-07-03). Only the correlated pairs need specifying; independence is the default and is often wrong for parameters that came out of the same fit.
...
- **Rows must sum to exactly 1.** heemod checks this loudly: `check_matrix()` runs inside `run_model()` and on every `run_psa()` draw, and any cell outside [0, 1] stops the run with `Some transition probabilities are outside the interval [0 - 1]` — it just won't say which conversion was wrong. The genuinely silent case is a row *sum* off by less than `all.equal()`'s ~1.5e-8 tolerance. Use `C` instead of computing the complement by hand wherever possible.
