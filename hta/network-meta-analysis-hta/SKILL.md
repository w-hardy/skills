...
- **Constant relative effect** (log OR / log HR): apply to a baseline to get each treatment's absolute effect; a PH log-HR can go straight into `hesim` via `params_surv_list` (the `multistate-models-hta` skill) or scale survival curves (the `survival-analysis-hta` skill). **Where the baseline comes from is a synthesis problem in its own right, not a lookup** — it must describe the *decision* population rather than the trials' control arms, the combination happens on the linear-predictor scale, and the resulting absolute effects are correlated across treatments through the shared baseline and the shared network. See `references/baseline-and-absolute-effects.md`, which also covers multiparameter evidence synthesis (combining several evidence streams into one coherent, already-correlated set of model parameters) and baseline-risk adjustment.
- **Pooled effect or predictive distribution?** The posterior for the pooled `d_t` narrows as trials accumulate; the predictive distribution for a *new* exchangeable population cannot narrow below τ. If the decision population is not exchangeable with the synthesised trials — usually the case — the predictive distribution is the honest input to a probabilistic model. Using the pooled mean understates decision uncertainty and so understates EVPI, distorting the "is more research worthwhile" answer downstream.
...
- **Re-sampling each treatment's absolute effect from its own marginal summary.** The absolute effects are correlated through the shared baseline and the shared network; drawing them independently for PSA destroys that and mis-states every incremental quantity. Carry the joint draws.
