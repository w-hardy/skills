...
**Multiple imputation marginalisation (MIM)** belongs on the same menu as G-computation STC and is worth knowing exists. It addresses the identical situation — two studies, IPD on one arm-set and aggregate data on the other, a non-collapsible effect measure — and targets a **marginal** estimand in the AgD population, like G-computation STC, but reaches it through a Bayesian multiple-imputation construction: the missing individual-level outcomes in the AgD study are imputed from the IPD-fitted model and the marginal effect is computed and pooled across imputations. It carries the same two-study and AgD-population limitations as MAIC and STC, so it does not change the dominating message above. An implementation exists in the `outstandR` package. **Its interface is not described here** — it could not be verified against the package documentation when this file was written — so check the package's own docs before writing code against it, and treat MIM as a method to be aware of rather than a default.
